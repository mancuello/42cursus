...
static void	get_rgb(char *line, t_fd *fd_data)
{
	...
	{
		fd_data->floor_color = arr[1];
		fd_data->elements++;
	}
	else if (arr[0][0] == 'C' && ft_strlen(arr[0]) == 1)
	{
		fd_data->ceiling_color = arr[1];
		fd_data->elements++;
	}
	free(arr[0]);
	free(arr[1]);
...
static void assign_path(char **arr, t_fd *fd_data)
{
	...
	else if ((arr[0][0] == 'S' && arr[0][1] == 'O' && ft_strlen(arr[0]) == 2) || (arr[0][0] == 'S' && ft_strlen(arr[0]) == 1))
	{
		fd_data->no_texture = arr[1];
		fd_data->elements++;
	}
	else if ((arr[0][0] == 'W' && arr[0][1] == 'E' && ft_strlen(arr[0]) == 2) || (arr[0][0] == 'W' && ft_strlen(arr[0]) == 1))
	{
		fd_data->no_texture = arr[1];
		fd_data->elements++;
	}
	else if ((arr[0][0] == 'E' && arr[0][1] == 'A' && ft_strlen(arr[0]) == 2) || (arr[0][0] == 'A' && ft_strlen(arr[0]) == 1))
	{
		fd_data->no_texture = arr[1];
		fd_data->elements++;
	}
